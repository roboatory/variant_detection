...
3. **Read the in-scope files**: The relevant code lives in `src/models/`. Read these files for full context:
   - `prepare.py` — fixed constants, data loading, dataset, evaluation, metrics. Do not modify.
   - `train.py` — the file you modify. Model architecture, optimizer, hyperparameters, training loop.
4. **Verify data exists**: Check that `data/features/{training,validation,test}/matrices/` contain `.npy` files and that each split has a `labels.txt`.
5. **Initialize results.tsv**: Create `results.tsv` in `src/models/` with just the header row. The baseline will be recorded after the first run.
...
```
---
val_elementwise_f1:     0.850000
val_any_sv_f1:          0.900000
val_exact_match:        0.750000
val_loss:               0.350000
...
```
grep "^val_elementwise_f1:" run.log
```
...
```
commit	val_elementwise_f1	val_any_sv_f1	status	description
```

1. git commit hash (short, 7 chars)
2. val_elementwise_f1 achieved (e.g. 0.850000) — use 0.000000 for crashes
3. val_any_sv_f1 achieved (e.g. 0.900000) — use 0.000000 for crashes
...
b2c3d4e	0.870000	0.920000	keep	increase LR to 3e-4
c3d4e5f	0.840000	0.890000	discard	switch to SGD optimizer
d4e5f6g	0.000000	0.000000	crash	double embedding dimension (OOM)
```
...
3. git commit
4. Run the experiment: `cd src/models && uv run python train.py > run.log 2>&1` (redirect everything — do NOT use tee or let output flood your context)
5. Read out the results: `grep "^val_elementwise_f1:\|^val_any_sv_f1:\|^val_loss:" run.log`
6. If the grep output is empty, the run crashed. Run `tail -n 50 run.log` to read the Python stack trace and attempt a fix. If you can't get things to work after more than a few attempts, give up.
7. Record the results in the tsv (NOTE: do not commit the results.tsv file, leave it untracked by git)
8. If val_elementwise_f1 improved (higher), you "advance" the branch, keeping the git commit
9. If val_elementwise_f1 is equal or worse, you git reset back to where you started
